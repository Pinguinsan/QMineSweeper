#ifndef QMINESWEEPER_PROGRAMOPTION_HPP
#define QMINESWEEPER_PROGRAMOPTION_HPP
...
class ProgramOption {
public:
    inline ProgramOption(char shortOption, const std::string &longOption, int argumentSpecifier,
    ...
    inline void setShortOption(int shortOption) {
        if (shortOption == 0) {
            throw std::runtime_error(
                    "ProgramOption::setShortOption(int shortOption): Short option must be a non-zero integer value");
        }
    ...
    inline void setLongOption(const std::string &longOption) {
        if (longOption.length() == 0) {
            throw std::runtime_error(
                    "ProgramOption::setLongOption(const std::string &): Long option must be at least 1 characters in length");
        }
        this->m_longOption = longOption;
    }

    inline void setArgumentSpecifier(int argumentSpecifier) {
        if ((argumentSpecifier != no_argument) && (argumentSpecifier != optional_argument) &&
            (argumentSpecifier != required_argument)) {
            std::ostringstream message{};
            message
                    << "ProgramOption::setArgumentSpecifier(int): Argument specifier must be either no_argument, optional_argument, or required_argument ("
    ...
    inline void setDescription(const std::string &description) {
        if (description.length() == 0) {
            throw std::runtime_error(
                    "ProgramOption::setDescription(const std::string &str): Description must be at least 1 character long");
        }
    ...
    inline option toPosixOption() const {
        if ((this->m_longOption[0] == '-') && (this->m_longOption[1] == '-')) {
            return option{this->m_longOption.substr(2).c_str(), this->m_argumentSpecifier, nullptr,
                          this->m_shortOption};
        } else if (this->m_longOption[0] == '-') {
            return option{this->m_longOption.substr(1).c_str(), this->m_argumentSpecifier, nullptr,
                          this->m_shortOption};
        } else {
    ...
    static std::string buildShortOptions(const Container &container) {
...
#endif //QMINESWEEPER_PROGRAMOPTION_HPP
