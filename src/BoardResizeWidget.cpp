...
BoardResizeWidget::BoardResizeWidget(QWidget *parent) :
    ...
    this->connect(this->m_ui->btnIncrementRows, &QPushButton::clicked, this,
                  &BoardResizeWidget::onBtnIncrementRowsClicked);
    this->connect(this->m_ui->btnDecrementRows, &QPushButton::clicked, this,
                  &BoardResizeWidget::onBtnDecrementRowsClicked);
    this->connect(this->m_ui->btnIncrementColumns, &QPushButton::clicked, this,
                  &BoardResizeWidget::onBtnIncrementColumnsClicked);
    this->connect(this->m_ui->btnDecrementColumns, &QPushButton::clicked, this,
                  &BoardResizeWidget::onBtnDecrementColumnsClicked);

    this->connect(this->m_ui->btnBeginner, &QPushButton::clicked, this, &BoardResizeWidget::onBtnBeginnerPresetClicked);
    this->connect(this->m_ui->btnIntermediate, &QPushButton::clicked, this,
                  &BoardResizeWidget::onBtnIntermediatePresetClicked);
    ...
    this->m_ui->btnBeginner->setToolTip(
            QString{this->m_ui->btnBeginner->toolTip()}.arg(
                    QString{"%1x%2"}.arg(QS_NUMBER(BEGINNER_GAME_DIMENSIONS.first),
                                         QS_NUMBER(BEGINNER_GAME_DIMENSIONS.second))
            )
    );

    this->m_ui->btnIntermediate->setToolTip(
            QString{this->m_ui->btnIntermediate->toolTip()}.arg(
                    QString{"%1x%2"}.arg(QS_NUMBER(INTERMEDIATE_GAME_DIMENSIONS.first),
                                         QS_NUMBER(INTERMEDIATE_GAME_DIMENSIONS.second))
            )
    );

    this->m_ui->btnAdvanced->setToolTip(
            QString{this->m_ui->btnAdvanced->toolTip()}.arg(
                    QString{"%1x%2"}.arg(QS_NUMBER(ADVANCED_GAME_DIMENSIONS.first),
                                         QS_NUMBER(ADVANCED_GAME_DIMENSIONS.second))
            )
    );

    this->m_ui->btnExtreme->setToolTip(
            QString{this->m_ui->btnExtreme->toolTip()}.arg(
                    QString{"%1x%2"}.arg(QS_NUMBER(EXTREME_GAME_DIMENSIONS.first),
                                         QS_NUMBER(EXTREME_GAME_DIMENSIONS.second))
            )
    );
...
void BoardResizeWidget::onPresetBoardSizeActionTriggered(QPushButton *pressedButton) {
    ...
    std::string rawString{
            pressedButton->property(QmsStrings::RESIZE_BOARD_BUTTON_GAME_DIMENSIONS_KEY).toString().toStdString()};
    std::transform(rawString.begin(), rawString.end(), rawString.begin(), ::tolower);
    rawString.erase(std::remove_if(rawString.begin(), rawString.end(),
                                   [](auto c) { return (!::isdigit(c) && (c != 'x') && (c != ' ')); }),
                    rawString.end());
    ...
    if ((dimensions.first == -1) || (dimensions.second == -1)) {
        throw std::runtime_error(
                QString{"In BoardResizeDialog::onBeginnerBoardSizeTriggered() : dimensions parsed from QPushButton menu item are not valid (QAction::text() = \"%1\", this should never happen)"}.arg(
                        maybeDimensions.c_str()).toStdString());
    }
...
void BoardResizeWidget::setMaximumColumns(int maximumColumns) {
    ...
    if (maximumColumns < this->m_minimumColumns) {
        throw std::runtime_error(
                "BoardResizeWidget::setMaximumColumns(int): column count cannot be less than current minimum column count (" +
                toStdString(maximumColumns) + " < " + toStdString(this->m_minimumColumns) + ")");
    } else if (maximumColumns < 0) {
        throw std::runtime_error(
                "BoardResizeWidget::setMaximumColumns(int): column count cannot be less than or equal to zero (" +
                toStdString(maximumColumns) + " <= 0");
    }
...
void BoardResizeWidget::setMaximumRows(int maximumRows) {
    ...
    if (maximumRows < this->m_minimumRows) {
        throw std::runtime_error(
                "BoardResizeWidget::setMaximumRows(int): row count cannot be less than current minimum row count (" +
                toStdString(maximumRows) + " < " + toStdString(this->m_minimumRows) + ")");
    } else if (maximumRows < 0) {
        throw std::runtime_error(
                "BoardResizeWidget::setMaximumRows(int): row count cannot be less than or equal to zero (" +
                toStdString(maximumRows) + " <= 0");
    }
...
void BoardResizeWidget::setMinimumColumns(int minimumColumns) {
    ...
    if (minimumColumns > this->m_maximumColumns) {
        throw std::runtime_error(
                "BoardResizeWidget::setMinimumColumns(int): column count cannot be greater than current maximum column count (" +
                toStdString(minimumColumns) + " < " + toStdString(this->m_maximumColumns) + ")");
    } else if (minimumColumns < 0) {
        throw std::runtime_error(
                "BoardResizeWidget::setMinimumColumns(int): column count cannot be less than or equal to zero (" +
                toStdString(minimumColumns) + " <= 0");
    }
...
void BoardResizeWidget::setMinimumRows(int minimumRows) {
    ...
    if (minimumRows > this->m_maximumRows) {
        throw std::runtime_error(
                "BoardResizeWidget::setMinimumRows(int): row count cannot be greater than current maximum row count (" +
                toStdString(minimumRows) + " < " + toStdString(this->m_maximumRows) + ")");
    } else if (minimumRows < 0) {
        throw std::runtime_error(
                "BoardResizeWidget::setMinimumRows(int): row count cannot be less than or equal to zero (" +
                toStdString(minimumRows) + " <= 0");
    }
